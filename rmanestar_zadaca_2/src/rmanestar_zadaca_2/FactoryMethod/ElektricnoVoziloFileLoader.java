...
public class ElektricnoVoziloFileLoader extends FileLoader {
    ...
    public void parseLine(String linija) {
        ...
        if (linija.matches("^([1-9]+[0-9]*|[0]); .+; ([1-9]+[0-9]*|[0]); ([1-9]+[0-9]*|[0])$") && smCounter==3) {
            ...
            PostavkeSingleton.ps.println("Datoteka VOZILA  - Pogresan format linija: " + linija);
            String[] lista = linija.split("; ");

            switch (smCounter) {
                case 3:
                    if (!lista[0].matches("^([1-9]+[0-9]*|[0])$")) 
                        PostavkeSingleton.ps.println("Atribut ID VOZILA nije ispravan. Mora biti broj veci ili jednak 0");                  
                    if (!lista[1].matches("^.+$")) 
                        PostavkeSingleton.ps.println("Atribut NAZIV VOZILA nije ispravan. Mora biti niz znakova.");                  
                    if (!lista[2].matches("^([1-9]+[0-9]*|[0])$")) 
                        PostavkeSingleton.ps.println("Atribut VRIJEME PUNJENJA BATERIJE nije ispravan. Mora biti broj veci ili jednak 0");                  
                    if (!lista[3].matches("^([1-9]+[0-9]*|[0])$")) 
                        PostavkeSingleton.ps.println("Atribut DOMET nije ispravan. Mora biti broj veci ili jednak 0");                  
